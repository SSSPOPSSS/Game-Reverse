...
PMAPS readmaps_code_app();		// 读取maps文件
...
float getFloat(int pid, long int addr);	// 读取float类型
int getDword(int pid, long int addr);	// 读取dword类型
long int getZZ(int pid, long int addr);	// 读取指针
int getRoot(char *argv[]);		// 获取root权限
long int get_module_base(int pid, const char *module_name);	// so入口函数

float getFloat(int pid, long int addr)
{
	float var[4] = { 0 };
	int handle;
	char lj[64];
...
int getDword(int pid, long int addr)
{
	int var[4] = { 0 };
	int handle;
	char lj[64];
...
void OrngTechnology(char *argv[], char *b, char *mode)
{
	strcpy(bm, b);
	...
	printf("\033[33;31mWelcome to use OrngTools1.0\n\33[0m");
	printf("\033[33;32m初始化成功,请勿使用盗版 \n\33[0m");
	printf("\033[33;33m禁止倒卖！禁止外传！谢谢！\n\33[0m");
	printf("\033[33;34mAuthor©️BF\n\33[0m");
	printf("\033[33;35m安卓开发群:785870583\n\33[0m");
	printf("\033[33;36mAuthor©️BF:547087912\n\33[0m");
